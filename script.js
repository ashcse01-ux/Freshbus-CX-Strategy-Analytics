...
(function () {
  ...
  function renderFlatExcelTable(data) {
    ...
    // Collapsible Logic: 
    // Build Hierarchical Tree: Absolute Gregorian Calendar Mapping
    let calendar = {};
    for (let m = 0; m < 12; m++) {
        calendar[m] = { name: "", mtdIndex: -1, weeks: [] };
    }
    
    // Helper to get month index from summary name (e.g. "Jan MTD" or "W1 Jan")
    const monthMap = {"Jan":0,"Feb":1,"Mar":2,"Apr":3,"May":4,"Jun":5,"Jul":6,"Aug":7,"Sep":8,"Oct":9,"Nov":10,"Dec":11};
    
    // First Pass: Find all Summaries and build buckets
    for (let i = 0; i < data.columns.length; i++) {
        ...
        if (isSummary) {
            if (cName.includes('MTD')) {
                let mStr = cName.replace('MTD', '').trim();
                let mIdx = monthMap[mStr];
                if (mIdx !== undefined) {
                    calendar[mIdx].mtdIndex = i;
                    calendar[mIdx].name = cName;
                }
            } else if (cName.match(/^W\d/)) {
                let parts = cName.split(' ');
                let wNum = parseInt(parts[0].replace('W', ''));
                let mStr = parts[1];
                let mIdx = monthMap[mStr];
                if (mIdx !== undefined) {
                    while (calendar[mIdx].weeks.length < 4) {
                        calendar[mIdx].weeks.push({ name: `W${calendar[mIdx].weeks.length + 1} ${mStr}`, origIndex: -1, days: [] });
                    }
                    calendar[mIdx].weeks[wNum - 1].origIndex = i;
                    calendar[mIdx].weeks[wNum - 1].name = cName;
                }
            }
        }
    }
    
    // Second Pass: Distribute Days using pure math
    for (let i = 0; i < data.columns.length; i++) {
        if (!colVisible[i]) continue;
        let cName = data.columns[i];
        let cDate = data.dates[i];
        ...
        if (!isSummary) {
            ...
            if (!isNaN(d)) {
                let mIdx = d.getMonth();
                let dom = d.getDate();
                let weekNum = Math.ceil(dom / 7);
                if (weekNum > 4) weekNum = 4;
                
                // Ensure bucket exists
                while (calendar[mIdx].weeks.length < 4) {
                    let mStr = Object.keys(monthMap).find(key => monthMap[key] === mIdx) || "Month";
                    calendar[mIdx].weeks.push({ name: `W${calendar[mIdx].weeks.length + 1} ${mStr}`, origIndex: -1, days: [] });
                }
                
                calendar[mIdx].weeks[weekNum - 1].days.push({ name: cName, date: cDate, origIndex: i });
            }
        }
    }

    // Flatten tree into ordered nodes for rendering
    let orderedNodes = [];
    let monthCounter = 0;
    let weekCounter = 0;
    
    for (let m = 0; m < 12; m++) {
        let monthObj = calendar[m];
        
        let hasData = monthObj.mtdIndex !== -1 || monthObj.weeks.some(w => w.origIndex !== -1 || w.days.length > 0);
        if (!hasData) continue;
        
        const mId = `M${monthCounter++}`;
        
        if (monthObj.mtdIndex !== -1) {
            orderedNodes.push({ type: 'month', origIndex: monthObj.mtdIndex, name: monthObj.name, mId: mId });
        }
        
        for (let w = 0; w < monthObj.weeks.length; w++) {
            let weekObj = monthObj.weeks[w];
            let hasWeekData = weekObj.origIndex !== -1 || weekObj.days.length > 0;
            if (!hasWeekData) continue;
            
            const wId = `W${weekCounter++}`;
            
            if (weekObj.origIndex !== -1) {
                orderedNodes.push({ type: 'week', origIndex: weekObj.origIndex, name: weekObj.name, mId: mId, wId: wId });
            }
            
            // Sort days to guarantee correct chronological order in UI
            weekObj.days.sort((a, b) => new Date(a.date) - new Date(b.date));
            
            for (let d = 0; d < weekObj.days.length; d++) {
                let dayObj = weekObj.days[d];
                orderedNodes.push({ type: 'day', origIndex: dayObj.origIndex, name: dayObj.name, date: dayObj.date, mId: mId, wId: wId });
            }
        }
    }
    ...
    orderedNodes.forEach(node => {
        let i = node.origIndex;
        let cName = data.columns[i];
        let cDate = data.dates[i];
        
        if (node.type === 'month') {
            let bgStyle = 'background:rgba(16,185,129,0.8);'; // Green for MTD
            trDates.innerHTML += `<th class="summary-col col-month" data-month="${node.mId}" rowspan="2" style="${bgStyle}; cursor:pointer;">${cName} <span class="expand-icon">▶</span></th>`;
        } 
        else if (node.type === 'week') {
            let bgStyle = 'background:rgba(26,115,232,0.5);'; // Blue for Week
            trDates.innerHTML += `<th class="summary-col col-week grp-month-${node.mId} col-collapsed" data-week="${node.wId}" rowspan="2" style="${bgStyle}; cursor:pointer;">${cName} <span class="expand-icon">▶</span></th>`;
        }
        else if (node.type === 'day') {
            let bgStyle = '';
            trDates.innerHTML += `<th class="day-col grp-month-${node.mId} grp-week-${node.wId} col-collapsed" style="${bgStyle}">${cDate}</th>`;
            trCols.innerHTML += `<th class="day-col grp-month-${node.mId} grp-week-${node.wId} col-collapsed" style="${bgStyle}">${cName}</th>`;
        }
    });
    ...
    data.metrics.forEach(m => {
        ...
        orderedNodes.forEach(node => {
            ...
            if (node.type === 'month') {
                let bgStyle = 'background:rgba(16,185,129,0.1); font-weight:bold;';
                tr.innerHTML += `<td class="summary-col col-month" style="${bgStyle}">${val}</td>`;
            } 
            else if (node.type === 'week') {
                let bgStyle = 'background:rgba(26,115,232,0.1); font-weight:bold;';
                tr.innerHTML += `<td class="summary-col col-week grp-month-${node.mId} col-collapsed" style="${bgStyle}">${val}</td>`;
            }
            else if (node.type === 'day') {
                let bgStyle = '';
                tr.innerHTML += `<td class="day-col grp-month-${node.mId} grp-week-${node.wId} col-collapsed" style="${bgStyle}">${val}</td>`;
            }
        });
        tbody.appendChild(tr);
    });

    // Add Collapse/Expand interactions
    // Month Toggle
    excelTable.querySelectorAll('th.col-month[data-month]').forEach(th => {
        th.classList.add('collapsed');
        th.addEventListener('click', () => {
            const mId = th.getAttribute('data-month');
            const icon = th.querySelector('.expand-icon');
            if(!icon) return;
            
            const isCollapsed = th.classList.contains('collapsed');
            if (isCollapsed) {
                // Expanding month -> show weeks
                th.classList.remove('collapsed');
                icon.innerHTML = '▼';
                excelTable.querySelectorAll(`th.col-week.grp-month-${mId}, td.col-week.grp-month-${mId}`).forEach(el => {
                    el.classList.remove('col-collapsed');
                });
            } else {
                // Collapsing month -> hide EVERYTHING in month
                th.classList.add('collapsed');
                icon.innerHTML = '▶';
                excelTable.querySelectorAll(`.grp-month-${mId}`).forEach(el => {
                    if (el !== th) el.classList.add('col-collapsed');
                });
                // Reset week chevrons
                excelTable.querySelectorAll(`th.col-week.grp-month-${mId}`).forEach(wTh => {
                    wTh.classList.add('collapsed');
                    let wIcon = wTh.querySelector('.expand-icon');
                    if (wIcon) wIcon.innerHTML = '▶';
                });
            }
        });
    });
    ...
    // Week Toggle
    excelTable.querySelectorAll('th.col-week[data-week]').forEach(th => {
        th.classList.add('collapsed');
        th.addEventListener('click', () => {
            const wId = th.getAttribute('data-week');
            const icon = th.querySelector('.expand-icon');
            if(!icon) return;
            
            const isCollapsed = th.classList.contains('collapsed');
            if (isCollapsed) {
                // Expanding week -> show days
                th.classList.remove('collapsed');
                icon.innerHTML = '▼';
                excelTable.querySelectorAll(`.day-col.grp-week-${wId}`).forEach(el => {
                    el.classList.remove('col-collapsed');
                });
            } else {
                // Collapsing week -> hide days
                th.classList.add('collapsed');
                icon.innerHTML = '▶';
                excelTable.querySelectorAll(`.day-col.grp-week-${wId}`).forEach(el => {
